...
'''Todo list
   - With -L (no symlinks), size keeps trying to get the size of files it can't get to
   ...
   - check_output() is a mess -- I don't like that I have different ways of handling sys.stdout vs filenames'''


def main():
    parser = make_parser()
    args = parser.parse_args()
    output = check_output(args.output_filename)     #Want to call check_output() before find_duplicates() because 
    h=find_duplicates(args.input_dirs, args.verbose, args.follow_links) #user can cancel execution in check_output() 
...
def build_size_dict(base_paths, verbose=False, follow_links=True) :
    ...
    sizemap=purge(sizemap) #Consider inlining purge so sizemap doesn't have to get passed around: For now, purge() only used here
    if verbose :
        print("Found {} unique filesizes that are duplicate candidates.".format(len(sizemap)))
    return sizemap


def purge(search_map) :
...
def make_parser() :
    ...
    parser.add_argument('-L', '--no-follow-links', dest='follow_links', help="Don't follow symlinks", action='store_false', default=True) #if -L is present, pass followlinks=false to os.walk
    return parser
